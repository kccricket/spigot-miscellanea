package net.kccricket.spigotmiscellanea.zcore.utils.loader;

import net.kccricket.spigotmiscellanea.exceptions.ItemEnchantException;
import net.kccricket.spigotmiscellanea.exceptions.ItemFlagException;
...
public class ItemStackLoader extends ZUtils implements Loader<ItemStack> {
    ...
    public void save(ItemStack item, YamlConfiguration configuration, String path) {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta.hasDisplayName()) configuration.set(path + "name", meta.getDisplayName().replace("§", "&"));
        if (meta.hasLore()) configuration.set(path + "lore", colorReverse(meta.getLore()));
